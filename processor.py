    ...
    if not files:
        ...
        if new_count >= MAX_ALBUM_RETRIES:
            await _update_album(album_id, status='error', retry_count=new_count,
                                error=f'Not found after {new_count} attempts — will auto re-check daily')
            await db.download_update(download_id, 'failed',
                                     error=f'Not found after {new_count} attempts — will auto re-check daily')
            await db.log('warn', f'Paused after {new_count} attempts (auto re-check in 24h): {artist_name} — {album_title}')
