    ...
    if not files:
        ...
        if new_count >= MAX_ALBUM_RETRIES:
            await _update_album(album_id, status='error', retry_count=new_count,
                                error=f'Not found after {new_count} attempts — will auto re-check weekly')
            await db.download_update(download_id, 'failed',
                                     error=f'Not found after {new_count} attempts — will auto re-check weekly')
            await db.log('warn', f'Paused after {new_count} attempts (auto re-check in 7d): {artist_name} — {album_title}')
            asyncio.create_task(send_push('⏸️ Download paused', f'{artist_name} — {album_title} (not found after {new_count} tries; will retry automatically)'))
        else:
...
async def scan_existing_library() -> dict:
    ...
    for artist_id, artist_name in artist_rows:
        ...
        for subdir in subdirs:
            ...
            if tag:
                # Stash the tag — even if it doesn't exact-match below, the
                # AI tiebreaker pass uses it instead of the folder name as
                # the disk-side identifier (ground truth from the file
                # itself beats a possibly-mangled folder name).
...
async def run_cycle():
    ...
    async with db.connect() as conn:
        ...
        # obtainable eventually. So a give-up is a *weekly re-check*, never a
        # permanent dead end: revive given-up albums for a fresh full-runway
        # attempt. Same connection + commit means the revived rows are included
        # in this cycle's SELECT below.
